...
### 设计规范
- **色彩方案**：主色调使用医疗蓝(#1976D2)搭配浅灰(#F5F7FA)背景
- **字体选择**：正文使用思源黑体，标题使用微软雅黑
- **响应式设计**：
  - 桌面端（≥1200px）：全功能展示
  - 平板端（768-1199px）：调整为单列布局
  - 移动端（<768px）：简化功能，分步骤引导
...
def create_main_interface():
    ...
    with st.sidebar:
        ...
        selected_tab = st.radio(
            "功能导航",
            ["上传图像", "查看分析", "治疗方案", "历史记录", "设置"]
        )
    
    # 主界面
    if selected_tab == "上传图像":
        st.header("图像上传")
        uploaded_file = st.file_uploader(
            "上传面部照片进行分析", 
            type=["jpg", "jpeg", "png"],
            help="请上传清晰的正面照片，避免佩戴眼镜或面部遮挡"
        )
        ...
        if uploaded_file is not None:
            ...
            with col1:
                st.image(image, caption="预览图像", use_column_width=True)
            with col2:
                st.markdown("### 分析选项")
                analysis_type = st.multiselect(
                    "选择需要分析的项目",
                    ["肤质状态", "皱纹分析", "色斑检测", "面部对称性", "全面评估"],
                    default=["全面评估"]
                )
                
                privacy_agreement = st.checkbox("我同意图像仅用于分析目的且将在7天后自动删除")
                
                if st.button("开始分析", disabled=not privacy_agreement):
                    with st.spinner("正在进行AI分析..."):
                        # 调用分析函数的占位
                        st.session_state.analysis_results = {"status": "completed"}
                    st.success("分析完成！")
                    st.session_state.current_tab = "查看分析"
```
...
> 详细的专业功能实现已经整理到单独的模块文档中，包含详尽的图像分析、专业报告生成和PDF输出等核心功能的完整实现代码。
> 
> **参见：** [专业功能.md](专业功能.md)
